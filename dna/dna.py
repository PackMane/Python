...
dna = dnalist[0]
# creates a dictionary where we will store the sequences we intend to count
sequences = {}

# extracts the sequences from the database into a list
...
# copies the list in a dictionary where the gene sequences are the keys
for item in dnaSequences:
    sequences[item] = 1

# iterates trough the dna sequence, when it finds repetitions of the values from sequence dictionary it counts them
for key in sequences:
    ...
    # stores the longest sequences in the dictionary using the correspondent key
    sequences[key] += tempMax

# opens and iterate trough the database of people treating each one like a dictionary so it can compare to the sequences one
